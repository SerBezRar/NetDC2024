...
- передача инкапсулированного VXLAN трафика полагается на IP-UDP протоколы, соответственно полагается на этот большой домен маршрутизации
...
В качестве Underlay выбран OSPF с P2P линками
Прежде всего за
...
---
10.x.LEAF-SERVICE-NUMBER.x (x - dont care в данном случае)
...
--- Логика нумерации для MACVRF не требуется - так как в Arista MACVRF по умолчанию создаётся по номеру vlan
Например 
vlan 10 = MACVRF10
