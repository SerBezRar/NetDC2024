...
--- Цели проекта
Цель проекта выбрана в качестве вероятного сценария ответа на внутренний запрос бизнеса текущей компании на разработку дизайна 

сети под предполагаемую новую локацию ЦОД.
Текущая локация насыщена по многим параметрам, которые указывают на необходимость разработки новой сети в новой локации
Стратегические Прогнозы бизнеса предполагают, что сеть должна поддерживать масштабирование до 50 стоек оборудования на ближайшие 

5 лет (в каждом может быть до 2х Top-of-Rack свичей для поддержки резервирования), т.е. до 100 Leaf свичей в сумме.

Соответственно цель проекта - выбор и разработка дизайна новой сети (Greenfield).
...
поддержка масштабирования и т.д.
...
Здесь и далее фабрика - новая разрабатываемая сеть.

сравнение производится с текущей сетью организации построенной на Cisco vPC + MSTP

Решает вопросы:
- extends Layer 2 segments beyond 4000 VLANs (нужно для multitenant DC) – примерно 16 миллионов.
- eliminates dependency on Spanning Tree Protocol,
- improves bandwidth utilization with equalcost multipath (ECMP) routing,
- provides Layer 2 adjacency, что важно через большое расстояния и L3 границы.

Проблемы – flood-and-learn форвардинг трафика, что при больших нагрузках проблемно.
Это и пытаются решить с помощью BGP-EVPN Control Plane.


Чем выглядит лучше:
- фабрика в сравнении с vPC
позволяет БОЛЬШУЮ отказоустойчивость ключевых элементов, через которые проходит весь трафик (Spine в данном случае). В vPC может 

быть только 2 экземпляра устройств.
позволяет БОЛЬШУЮ отвязанность от вендора в некоторых элементах (тех же Spine). vPC - проприетарный протокол. То что 

используется в фабрике в основном - открытые стандарты.
позволяет БОЛЕЕ ПРОСТОЕ масштабирование по полосе
позволяет эффективнее решать проблемы с петлями
устранение unknown unicast
увеличение количества broadcast доменов

=== Проектирование

Для выполнения проектирования адресного пространства в данном ДЗ остановимся подробнее на

    interface connection планировании (int plan)
    ip address and network underlay планировании (ip.addr-net plan)

Задание выполняется для сети клоза размером 2 spine 3 leaf но поддерживает расширение до бОльших размеров


Проектирования адресного пространства в данном ДЗ остановимся подробнее
на
